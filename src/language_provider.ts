...
export default class LogsQlLanguageProvider extends LanguageProvider {
  ...
  async getFieldList(options: FetchFieldsOptions, customParams?: URLSearchParams): Promise<FieldHits[]> {
    ...
    // Note: We use simple contains match instead of regex to avoid escaping issues
    // The fieldValueFilter is used for autocomplete suggestions, so we want to match
    // any value that contains the user's input as a substring
    let finalQuery = options.query || '*';
    if (options.type === FilterFieldType.FieldValue && options.field && options.fieldValueFilter) {
      // Use simple contains match (field:"value") instead of regex match
      // This avoids issues with regex special characters like * ? ( ) etc.
      // The contains match is more user-friendly for autocomplete suggestions
      const escapedValue = options.fieldValueFilter
        .replace(/\\/g, '\\\\')
        .replace(/"/g, '\\"');
      const fieldFilter = `${options.field}:"${escapedValue}"`;
